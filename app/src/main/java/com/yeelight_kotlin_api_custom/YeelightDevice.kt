...
data class YeelightDevice(
    ...
    suspend fun send(cmd: YeelightCmd): String? {
        var response: String? = null
        var attempt = 0
        val maxAttempts = 3 // Maximális újrapróbálkozások száma
        val timeoutMillis = 4000L // Timeout idő milliszekundumban

        while (response == null && attempt < maxAttempts) {
            try {
                withContext(Dispatchers.IO) {
                    val socket = aSocket(YeelightManager.ioSelector).tcp().connect(ip, port) {
                        socketTimeout = timeoutMillis.toInt().toLong()
                    }

                    socket.use { socket ->
                        val writeChannel = socket.openWriteChannel(autoFlush = true)
                        writeChannel.writeStringUtf8("${cmd.realCommand}\r\n")
                        if (enableLog) println("$id --> ${cmd.realCommand}")

                        val readChannel = socket.openReadChannel()
                        response = readChannel.readUTF8Line()
                        if (enableLog) println("$id <-- $response")
                    }
                }
