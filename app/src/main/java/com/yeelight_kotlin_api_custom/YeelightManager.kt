...
class YeelightManager(
    ...
    //saját kiegészítés, ha esetleg több eszközünk lenne, akkor ez egy kiindulási alap, pl: kilistázni.
    /* fun getAllYeelightDeviceIds(): List<String> {
        return findAllDevices()
            .map { it.id }
    }*/
    suspend fun getAllYeelightDeviceIds(): List<String> {
        return findAllDevices()
            .map { it.id.replace("\\[|\\]", "") }
    }
